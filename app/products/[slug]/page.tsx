...
const BUNDLE_PRODUCTS: Record<string, string[]> = {
  "bio-bloom-fertiliser": ["terra-pro-soil-mix",   "deep-root-tonic"  ],
  "terra-pro-soil-mix":   ["bio-bloom-fertiliser", "deep-root-tonic"  ],
  "deep-root-tonic":      ["bio-bloom-fertiliser", "terra-pro-soil-mix"],
  "season-starter-kit":   [],
};
...
export default function ProductPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const related   = DEMO_PRODUCTS.filter((p) => p.handle !== slug).slice(0, 3);
  const bundleHandles = BUNDLE_PRODUCTS[slug] ?? [];
  const bundleProducts = bundleHandles.map((h) => DEMO_PRODUCTS.find((p) => p.handle === h)).filter(Boolean);
            ...
            >
              ...
              <p className="font-bold mb-5" style={{ fontSize: "2rem", color: "var(--green-bio)" }}>
                ...
                {activeVariant === 0 && variants.length > 1 && (
                  <span className="text-sm font-normal ml-2" style={{ color: "var(--text-black-soft)" }}>
                    from
                  </span>
                )}
              </p>
              ...
              <div className="flex flex-col lg:flex-row gap-4 items-start lg:items-center">
                ...
                <div className="flex items-center gap-3">
                  <div
                    ...
                    style={{ background: (GALLERY_BG[slug] ?? GALLERY_BG["bio-bloom-fertiliser"])[0], boxShadow: "var(--shadow-card)" }}
                  >
                    <div className="w-full h-full flex items-center justify-center scale-75 opacity-80">
                ...
                {bundleProducts.map((bp) => {
                  ...
                  return (
                    <div key={bp.handle} className="flex items-center gap-3">
                      ...
                      <div
                        className="w-20 h-20 rounded-xl overflow-hidden flex-shrink-0"
                        style={{ background: (GALLERY_BG[bp.handle] ?? GALLERY_BG["bio-bloom-fertiliser"])[0], boxShadow: "var(--shadow-card)" }}
                      >
                        <div className="w-full h-full flex items-center justify-center scale-75 opacity-80">
                          <svg viewBox="0 0 80 100" xmlns="http://www.w3.org/2000/svg" className="w-10" aria-hidden="true">
                            <rect x="8" y="20" width="64" height="75" rx="4" fill="#1E3932" />
